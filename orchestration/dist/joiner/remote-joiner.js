...
class RemoteJoiner {
    ...
    async handleExpands(items, parsedExpands, implodeMapping = []) {
        if (!parsedExpands) {
            return;
        }
    ...
    parseExpands(initialService, query, serviceConfig, expands, implodeMapping) {
        const parsedExpands = this.parseProperties(initialService, query, serviceConfig, expands, implodeMapping);
        const groupedExpands = this.groupExpands(parsedExpands);
        return groupedExpands;
    }
    parseProperties(initialService, query, serviceConfig, expands, implodeMapping) {
    ...
    parseAlias({ aliasPath, aliasRealPathMap, expands, expand, property, parsedExpands, currentServiceConfig, currentPath, implodeMapping, forwardArgumentsOnPath, }) {
        ...
        implodeMapping.push({
            ...
            property,
            path: fullPath,
            isList: !!serviceConfig.relationships?.find((relationship) => relationship.alias === parentFieldAlias)?.isList,
        });
    ...
    async query(queryObj) {
        ...
        await this.handleExpands(Array.isArray(data) ? data : [data], parsedExpands, implodeMapping);
