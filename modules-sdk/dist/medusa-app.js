...
async function loadModules(modulesConfig, sharedContainer, migrationOnly = false, loaderOnly = false) {
    const allModules = {};
...
async function MedusaApp_({ sharedContainer, sharedResourcesConfig, servicesConfig, modulesConfigPath, modulesConfigFileName, modulesConfig, linkModules, remoteFetchData, injectedDependencies = {}, onApplicationStartCb, migrationOnly = false, loaderOnly = false, } = {}) {
    ...
    const allModules = await loadModules(modules, sharedContainer_, migrationOnly, loaderOnly);
    if (loaderOnly) {
        return {
            modules: allModules,
            link: undefined,
